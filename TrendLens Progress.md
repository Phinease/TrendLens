...
**目标：** 打开 App 能看到 Feed 页面显示新卡片，可上下滚动
...
- [ ] **2.2 重构 FeedView.swift**
  ...
  - [ ] 实现 Hero + Standard 混合布局
    - [ ] Rank 1-3：使用 HeroCard
    - [ ] Rank 4+：使用 StandardCard
  - [ ] 保留原有的数据流和 ViewModel（不改业务逻辑）
  - [ ] 更新列表间距（Hero 间: 16pt, Standard 间: 12pt）
  - [ ] 页面背景改为 `.backgroundPrimary`
  - [ ] 底部留白 80pt（为后续 FloatingDock 预留空间）
  - [ ] 保留现有平台 Tab（暂时保持，Phase 3 升级为 FluidRibbon）
  - [ ] 测试滚动流畅度

- [ ] **2.3 临时验证**
  - [ ] 删除 CardGalleryView（不再需要）
  - [ ] 在 `TrendLensApp.swift` 恢复为 Feed 页面
  - [ ] 运行 Simulator，验证卡片显示和滚动
