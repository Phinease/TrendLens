...
- **Swift 6.2**（默认 `@MainActor` 隔离，Strict Concurrency Checking: Complete）

> ⚠️ **Swift 6.2 重要变化**：新项目默认所有代码运行在 `@MainActor`，需要后台执行时显式标记 `nonisolated`。详见《TrendLens Technical Architecture.md》。
...
- **SwiftData（本地缓存/离线）**
  ...
  - **iOS 26 新增**：Model Inheritance（模型继承）、Persistent History（变更追踪）
